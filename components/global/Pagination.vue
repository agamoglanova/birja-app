  ...
  <div class="pagination">
    <div class="pagination__content" v-if="totalElems > perPage">
      ...
      >
        <img
          alt="Назад"
          src="/pics/svg/arrow-right-gray.svg"
      ...
      <button
        ...
        :class="{ 'three-dots': button == '. . .', current: button == value }"
        :disabled="button == '. . .' || button == value"
      >
        <span>{{ button }}</span>
      </button>
      <button
      ...
      <button
        ...
        :disabled="value == lastPage"
      >
        <img alt="Вперед" src="/pics/svg/arrow-right-gray.svg" data-not-lazy />
      </button>
    </div>
    <div v-if="totalElems == 0">
      <span class="pagination__empty" v-if="emptyText">{{
        $t(emptyText)
      }}</span>
    </div>
  </div>
